		...
		while (getline(in, line)) {
			if (!getbalance) {
				...
				if (q < 3) {
					...
					temp.push_back(stoi(line));
					DinoStr* dinostr = new DinoStr(temp[1], temp[2], temp[3]);
					DinoDex* dinodex = new DinoDex(temp[1], temp[2], temp[3]);
					DinoInt* dinoint = new DinoInt(temp[1], temp[2], temp[3]);

					switch (temp[0]) {
					case 1:
						dinos.push_back(dinostr);
						break;
					case 2:
						dinos.push_back(dinodex);
						break;
					case 3:
						dinos.push_back(dinoint);
						break;
					default:
						break;
					}
			...
			if (shop) {
				...
				while (SDL_PollEvent(&event)) {
					...
					if (event.type == SDL_MOUSEBUTTONDOWN) {
						...
						if (!shopact) {
							if (balance > 0) {
								DinoStr* dinostr = new DinoStr(shopdino[1], shopdino[2], shopdino[3]);
								DinoDex* dinodex = new DinoDex(shopdino[1], shopdino[2], shopdino[3]);
								DinoInt* dinointe = new DinoInt(shopdino[1], shopdino[2], shopdino[3]);
								switch (shopdino[0]) {
								case 1:
									dinos.push_back(dinostr);
									break;
						...
						if (go) {
							...
							if (sum > 10) {
								for (int i = 0; i < dinochoose.size(); i++) {
									if (dinochoose.at(i) == 3) {
										DinoStr* dinostr = new DinoStr(dinos[0]->str, dinos[0]->dex, dinos[0]->inte);
										DinoDex* dinodex = new DinoDex(dinos[0]->str, dinos[0]->dex, dinos[0]->inte);
										DinoInt* dinoint = new DinoInt(dinos[0]->str, dinos[0]->dex, dinos[0]->inte);

										switch (dinos[0]->type) {
										case 1:
											yourpull.push_back(dinostr);
											break;
										case 2:
											yourpull.push_back(dinodex);
											break;
										case 3:
											yourpull.push_back(dinoint);
											break;
										default:
											break;
										}
										...
										DinoStr* dinostro = new DinoStr(temp[1], temp[2], temp[3]);
										DinoDex* dinodexo = new DinoDex(temp[1], temp[2], temp[3]);
										DinoInt* dinointo = new DinoInt(temp[1], temp[2], temp[3]);

										switch (temp[0]) {
										case 1:
											enemypull.push_back(dinostro);
											break;
										case 2:
											enemypull.push_back(dinodexo);
											break;
										case 3:
											enemypull.push_back(dinointo);
											break;
										default:
											break;
										}
				...
				while (SDL_PollEvent(&event)) {
					...
					if (event.type == SDL_MOUSEBUTTONDOWN) {
						...
						if (you) {
							...
							switch (place) {
							case 1:
								enemypull[0]->hp -= yourpull[0]->damage(place) * th;
								break;
							case 2:
								enemypull[0]->hp -= yourpull[0]->damage(place) * th;
								break;
